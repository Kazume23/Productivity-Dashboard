const todoEmpty = $("todoEmpty");
const todoList = $("todoList");
const todoAddBtn = $("todoAddBtn");
const todoTitleSub = $("todoTitleSub");
const todoOverlay = $("todoOverlay");
const todoDateInput = $("todoDate");
const todoText = $("todoText");
const todoPriority = $("todoPriority");
const todoRecurrence = $("todoRecurrence");
const todoChecklist = $("todoChecklist");
const todoSave = $("todoSave");
const todoClose = $("todoClose");
const todoCancel = $("todoCancel");
const todoKanbanBoard = $("todoKanbanBoard");
const todoKanbanSummary = $("todoKanbanSummary");

const calTitle = $("calTitle");
const calGrid = $("calGrid");
const btnPrev = $("calPrev");
const btnNext = $("calNext");
const btnToday = $("calToday");

const habRange = $("habRange");
const habThead = $("habThead");
const habTbody = $("habTbody");
const habitName = $("habitName");
const addHabitBtn = $("addHabit");
const habitOverlay = $("habitOverlay");
const habitModalName = $("habitModalName");
const habitSave = $("habitSave");
const habitClose = $("habitClose");
const habitCancel = $("habitCancel");
const habitTemplateSelect = $("habitTemplateSelect");
const habitTemplateAdd = $("habitTemplateAdd");
const habitGoalWeek = $("habitGoalWeek");
const habitGoalMonth = $("habitGoalMonth");
const habitGoalSave = $("habitGoalSave");
const habitGoalWeekProgress = $("habitGoalWeekProgress");
const habitGoalMonthProgress = $("habitGoalMonthProgress");
const habitFreezeHabit = $("habitFreezeHabit");
const habitFreezeLimit = $("habitFreezeLimit");
const habitFreezeSave = $("habitFreezeSave");
const habitFreezeUse = $("habitFreezeUse");
const habitFreezeCredits = $("habitFreezeCredits");
const habitFreezeInfo = $("habitFreezeInfo");
const habitRemindersEnabled = $("habitRemindersEnabled");
const habitReminderStartHour = $("habitReminderStartHour");
const habitReminderEndHour = $("habitReminderEndHour");
const habitReminderDailyLimit = $("habitReminderDailyLimit");
const habitReminderCooldown = $("habitReminderCooldown");
const habitReminderSave = $("habitReminderSave");
const habitReminderPermission = $("habitReminderPermission");
const habitReminderStatus = $("habitReminderStatus");

const chartCanvas = $("chartCanvas");
const chartWeekBtn = $("chartWeek");
const chartMonthBtn = $("chartMonth");
const chartRangeTxt = $("chartRangeTxt");
const chartDoneTxt = $("chartDone");
const chartFailTxt = $("chartFail");
const chartEmptyTxt = $("chartEmpty");
const chartDetailsBtn = $("chartDetailsBtn");
const chartOverlay = $("chartOverlay");
const chartClose = $("chartClose");
const chartModalRange = $("chartModalRange");
const chartModalSummary = $("chartModalSummary");
const chartModalList = $("chartModalList");
const habitsBarChartCanvas = $("habitsBarChart");
const habitsCompareChartCanvas = $("habitsCompareChart");

const dashProductivityChartCanvas = $("dashProductivityChart");
const dashSpendTrendChartCanvas = $("dashSpendTrendChart");

const dashQuickTodoText = $("dashQuickTodoText");
const dashQuickTodoPriority = $("dashQuickTodoPriority");
const dashQuickTodoAdd = $("dashQuickTodoAdd");
const dashQuickExpenseAmount = $("dashQuickExpenseAmount");
const dashQuickExpenseCategory = $("dashQuickExpenseCategory");
const dashQuickExpenseWhat = $("dashQuickExpenseWhat");
const dashQuickExpenseAdd = $("dashQuickExpenseAdd");
const dashQuickHint = $("dashQuickHint");

const expOnlySelected = $("expOnlySelected");
const expAmount = $("expAmount");
const expWhat = $("expWhat");
const expCategory = $("expCategory");
const expScore = $("expScore");
const expPeriod = $("expPeriod");
const expDate = $("expDate");
const expAdd = $("expAdd");
const expList = $("expList");
const expSummary = $("expSummary");
const expFilterCategory = $("expFilterCategory");
const expCategoryChartCanvas = $("expCategoryChart");
const expTrendChartCanvas = $("expTrendChart");
const expBudgetCategory = $("expBudgetCategory");
const expBudgetLimit = $("expBudgetLimit");
const expBudgetAlertPct = $("expBudgetAlertPct");
const expBudgetSave = $("expBudgetSave");
const expBudgetSummary = $("expBudgetSummary");
const expBudgetList = $("expBudgetList");
const expRecurringName = $("expRecurringName");
const expRecurringAmount = $("expRecurringAmount");
const expRecurringCategory = $("expRecurringCategory");
const expRecurringPeriod = $("expRecurringPeriod");
const expRecurringNextDate = $("expRecurringNextDate");
const expRecurringActive = $("expRecurringActive");
const expRecurringAdd = $("expRecurringAdd");
const expRecurringSummary = $("expRecurringSummary");
const expRecurringList = $("expRecurringList");
const expGoalName = $("expGoalName");
const expGoalTarget = $("expGoalTarget");
const expGoalCurrent = $("expGoalCurrent");
const expGoalDeadline = $("expGoalDeadline");
const expGoalAdd = $("expGoalAdd");
const expGoalSummary = $("expGoalSummary");
const expGoalList = $("expGoalList");
const expImportFile = $("expImportFile");
const expExportFinanceJson = $("expExportFinanceJson");
const expExportExpensesCsv = $("expExportExpensesCsv");
const expExportFullBackup = $("expExportFullBackup");
const expBackupImportFile = $("expBackupImportFile");
const expImportReport = $("expImportReport");
const expCompareCurrent = $("expCompareCurrent");
const expComparePrev = $("expComparePrev");
const expCompareDelta = $("expCompareDelta");
const expCompareInsight = $("expCompareInsight");
const expUpcomingList = $("expUpcomingList");
const expOverlay = $("expOverlay");
const expModalAmount = $("expModalAmount");
const expModalWhat = $("expModalWhat");
const expModalCategory = $("expModalCategory");
const expModalScore = $("expModalScore");
const expModalPeriod = $("expModalPeriod");
const expModalDate = $("expModalDate");
const expSaveBtn = $("expSave");
const expCloseBtn = $("expClose");
const expCancelBtn = $("expCancel");

const wishName = $("wishName");
const wishPrice = $("wishPrice");
const wishAdd = $("wishAdd");
const wishList = $("wishList");
const wishSort = $("wishSort");
const wishOverlay = $("wishOverlay");
const wishModalName = $("wishModalName");
const wishModalPrice = $("wishModalPrice");
const wishSaveBtn = $("wishSave");
const wishCloseBtn = $("wishClose");
const wishCancelBtn = $("wishCancel");

const pomoFocusBtn = $("pomoFocus");
const pomoBreakBtn = $("pomoBreak");
const pomoLongBtn = $("pomoLong");
const pomoTimeEl = $("pomoTime");
const pomoStartBtn = $("pomoStart");
const pomoResetBtn = $("pomoReset");
const pomoSessionEl = $("pomoSession");

const navDash = $("navDash");
const navHabits = $("navHabits");
const navTodo = $("navTodo");
const navExpenses = $("navExpenses");
const navWishlist = $("navWishlist");
const navAddHabits = $("navAddHabits");
const navAddTodo = $("navAddTodo");
const navAddExpenses = $("navAddExpenses");
const navAddWishlist = $("navAddWishlist");
const pageTitle = $("pageTitle");
const pageSubtitle = $("pageSubtitle");
const syncBadge = $("syncBadge");
const toastStack = $("toastStack");
const sidebar = $("sidebarNav");
const sidebarOverlay = $("sidebarOverlay");
const mobileMenuBtn = $("mobileMenuBtn");
const sidebarCloseBtn = $("sidebarCloseBtn");

const boxTopA = document.querySelector(".box.topA");
const tableBox = document.querySelector(".tableBox");
const bottomBox = document.querySelector(".bottomBox");
const todoBox = document.getElementById("todoBox");
const todoPanel = document.querySelector(".box.todoBox");
